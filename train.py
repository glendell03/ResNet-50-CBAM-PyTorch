...
def train(gpu, args):
    ...
    model = ResNet50(
        image_depth=args.img_depth, num_classes=args.num_classes, use_cbam=args.use_cbam
    )
    ...
    train_dataset = LoadDataset(
        dataset_folder_path=args.data_folder,
    ...
    for epoch_idx in range(args.epoch):
        ...
        for i, sample in tqdm(enumerate(train_generator)):
            batch_x, batch_y = sample["image"].cuda(non_blocking=True), sample[
                "label"
        ...
        with torch.set_grad_enabled(False):
            for i, sample in tqdm(enumerate(test_generator)):
                ...
                batch_accuracy = calculate_accuracy(
                    predicted=net_output, target=batch_y
                )
                epoch_loss.append(total_loss.item())
                epoch_accuracy.append(batch_accuracy)
